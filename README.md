...
A modular monolith only stays modular if something checks. That is the idea Kakehashi inherits, and
the only real change is that it now has to hold across a network as well as inside a process.

| Gate | What it protects | Command |
| --- | --- | --- |
| `archlint` | module boundaries **inside** the Go server | `cd server && go run ./tools/archlint` |
| `Kakehashi.ArchitectureTests` | the three layers **inside** the WinUI client | `cd client && dotnet test` |
...
The server's shape will look familiar if you have read [gtk-boilerplate](https://github.com/SekiroKenjii/gtk-boilerplate):
...
```pwsh
dotnet run --project client/src/App/Kakehashi.App/Kakehashi.App.csproj -p:Platform=x64
```
...
```pwsh
$env:KAKEHASHI_TEST_BACKEND = "http://localhost:8080"; dotnet test client/Kakehashi.slnx
```
...
```sh
cd server && CGO_ENABLED=0 GOOS=linux go build -o kakehashi ./cmd/server
```
