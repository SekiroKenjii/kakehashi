...
namespace Kakehashi.App.Infrastructure.DependencyInjection {
  // Observability is registered separately via AddObservability so the transport instrumentation
  // it adds is wired regardless of protocol.
  public static class InfrastructureServiceCollectionExtensions {
    public static IServiceCollection AddBackendInfrastructure(
      ...
      // Registered with TryAdd so the Auth module can replace it with a session-backed provider.
    ...
    // Feature modules call this from IModule.RegisterServices rather than assembling a channel
    // themselves. The token plumbing is the reason: a module that wired its own client and forgot
    // the call credentials would send unauthenticated requests, and would keep working right up
    ...
    public static IHttpClientBuilder AddBackendGrpcClient<TClient>(this IServiceCollection services)
      ...
      return services
          ...
          // reached over plain HTTP, so the guard has to be lifted or no token is ever sent; in
          // production TLS is terminated by the reverse proxy in front of it.
