...
func TestLoadDescriptor(t *testing.T) {
	...
	d, err := LoadDescriptor(dir, "1.2.1")
	if err != nil {
		t.Fatalf("LoadDescriptor: %v", err)
	}
	if d.TemplateVersion != "1.3.0" {
...
func TestLoadDescriptorChecksTheTemplateAgainstTheRangeTheCliDeclares(t *testing.T) {
	...
	_, err := LoadDescriptor(dir, "1.1.0")
	if err == nil {
	...
	for _, want := range []string{SupportedTemplates, "2.0.0", "upgrade the CLI"} {
...
func TestLoadDescriptorRefusals(t *testing.T) {
	cases := []struct {
		...
		{"a range that is not a range", `{"schemaVersion": 1, "templateVersion": "1.1.0",
		  "markersSchema": 1, "unitsSchema": 1, "requiresCli": "newest"}`},
		{"not json", `{`},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			if _, err := LoadDescriptor(templateDir(t, c.body), "1.1.0"); err == nil {
...
func TestLoadDescriptorSaysWhenADirectoryIsNotATemplate(t *testing.T) {
	_, err := LoadDescriptor(t.TempDir(), "1.1.0")
