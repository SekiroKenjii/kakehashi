...
// Matched by the column widths in store/.
const (
...
type Account struct {
	...
	Phone        string
	PasswordHash string
	// What the "team" row scope resolves against. Empty means the account belongs to no team, and a
	// team-scoped grant then reaches nothing — the safe reading.
	TeamID string

	// Zero until the account has signed in once. A zero time rather than a pointer because no rule
	// here distinguishes "never" from "long ago"; the store maps it to and from NULL at the edge,
	// where the distinction is the screen's to make.
	LastSignInAt time.Time

	// The switch an administrator throws instead of deleting: a deactivated account cannot
	// authenticate, and everything it did remains readable.
	IsActive bool
...
// Replaces the password without checking the old one, because an administrator does not have it.
// The protection is not a second factor but who may call it: the caller needs users.manage, and
// every reset revokes the account's sessions.
...
// Empty means no team, which narrows every team-scoped grant to nothing — a real choice, so it is
// allowed rather than silently ignored.
...
// Check after a successful sign-in, the only moment the plaintext is in hand and the hash can be
// upgraded without asking the user for anything.
...
// Only meaningful immediately after VerifyPassword returned true.
...
// A nil pointer leaves that field alone, so a caller can send a partial edit without having to
...
func normalizeEmail(email string) (string, error) {
	// Lower-cased because the address is the account's identity. The local part is technically
	// case-sensitive per RFC 5321; no mail provider anyone uses treats it that way, and honouring
	// it would create two accounts for one person.
...
// Length and nothing else. No "must contain a digit and a symbol": composition rules push people
// towards Passw0rd! and away from the long passphrases that are actually strong. NIST dropped them
// for this reason.
