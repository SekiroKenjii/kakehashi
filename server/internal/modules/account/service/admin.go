// Kept apart from profile.go because the caller is different: every use case there acts on the
// caller's own account and needs no permission beyond being signed in, and every one here acts on
// another person's and is reachable only behind users.manage.
...
// Ending every session is not a courtesy. A reset happens because somebody lost control of an
// account or left; leaving the existing sessions alive would change the password and change
// nothing else.
...
func (s *Service) RevokeAccountSession(ctx context.Context, accountID, sessionID string) error {
	...
	// into that account's own audit trail and told nobody else, leaving the activity feed silent
	// about the one event most worth its silence being broken.
...
// currentID marks the caller's own session, so an administrator inspecting themselves still sees
// "this device" on the row they are in.
...
// Permanent. Deactivation is the recommended path and the screen says so, because it keeps the
// history; this is for the record that must actually go away. The refusal to delete yourself
// mirrors the deactivation rule: an administrator who removes their own access needs a second
// administrator to notice, which is exactly when there may not be one.
...
// Revoking the sessions on the way down is the half that makes deactivation mean anything. Without
// it the account keeps working until its access token expires and its refresh token is next used —
// minutes during which the administrator believes they have switched somebody off and has not.
