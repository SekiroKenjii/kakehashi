...
// Append-only. Never edit one that has shipped: it is keyed by name, so changed SQL will not re-run
// on a database that already has it and the schema silently diverges. Add another.
