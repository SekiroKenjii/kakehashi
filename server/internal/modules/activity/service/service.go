...
//
// Nothing here imports another module: the whole vocabulary of this package is its own domain,
// which is what keeps its tests free of any other module's events.
package service
...
// Declared here rather than in store/: the interface belongs to the consumer, which is what lets
// these use cases be tested against a fake, and it keeps a package that must never import another
// module from naming anything but its own domain.
...
// Injected so tests can pin it instead of asserting on shapes.
...
// Absent from activityapi.Service on purpose: a feed another module can write into is a feed every
// module must call, which is the inverted dependency this module exists to avoid.
//
...
func (s *Service) List(
	...
	// One more than asked for: a page that comes back exactly full is otherwise indistinguishable
	// from the last one, and a "Load more" that leads to nothing is a worse bug than one that is
	// missing.
	...
	// comes off here — the store counts exactly what it is given.
	totals := filter
	totals.After = nil
	...
	if q.PageToken == "" {
		// One aggregation feeds both — the category totals are a fold over the per-kind numbers
...
// whole set rather than the filtered view — otherwise every chip but the active one reads zero.
// The range and the search still apply: those are what the reader chose to look at, the category
// is not.
...
// and silently getting 50 makes a client's paging arithmetic wrong in a way nothing reports. Zero
// still means the default, because that is what an unset field looks like on the wire.
...
// is not meant to be — it is a "do not depend on this" sign a client cannot miss.
...
// The border: nothing crosses out of the module without passing through here, and the user id
// stops at it, because it is the query's filter and never its result. The row id crosses — the
// reader of a row is the account that owns it.
//
// Category and Platform are computed here rather than stored, so a better answer applies to every
// row already written instead of only to the ones recorded after the improvement.
