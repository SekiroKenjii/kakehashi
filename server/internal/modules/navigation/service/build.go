...
// request. Nothing here decides anything about access — it reads the same answer the gate would
// give and turns it into a row that is present, disabled, or absent:
//
...
//	                             row tells an ordinary account nothing it can act on.
//
...
func (s *Service) Build(ctx context.Context, grants auth.Grants) (Pane, error) {
	...
	for _, d := range s.declared {
		...
		if !ok {
			...
			// Reconcile. Skipped rather than defaulted: a pane assembled from a layout that was
			// never written would disagree with the one every other client sees.
			continue
		}
...
// gateOf asks authzapi for the key rather than concatenating ".access" here: the format is the
// authorization module's contract, and a second copy stops matching the day it changes.
//
