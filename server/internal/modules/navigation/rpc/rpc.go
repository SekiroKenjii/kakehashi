// Package rpc is the navigation module's wire layer.
//
// It is the only package in the module allowed to import the generated protobuf code, and
// tools/archlint enforces that. Everything here is mapping, plus one thing that cannot be mapped:
// who is asking. That is read off the context, because a caller is not a field a client may send.
package rpc
...
// GetNavigation returns the pane this caller should draw.
//
// Open to any signed-in caller, and it has to be: this is the answer a client needs before it can
// draw anything at all, so gating it on a permission would mean a new account with no grants gets no
// pane and no way to ask for one.
//
// It reads the grants the gate already resolved onto the request rather than querying again. Two code
// paths that could disagree about what a caller may do is how a client ends up drawing an unlocked
// door onto a locked room.
