// Package errs gives the server a small, closed set of error kinds, so that deciding whether a
// caller sees 404, 400, 409 or a bare 500 needs no string-matching or type-assertion.
//
// The translation deliberately does not live here. This package imports nothing but the standard
...
// Write the message for the user, not the log: Invalid is one of the kinds whose text crosses the
// wire verbatim.
...
// Anything that is not an *Error is Internal, including nil, which no caller should be asking
// about.
...
// on, and rather more than we meant to tell them.
